...
def authenticate_user(email: str, password: str | None = None, role: str | None = None) -> dict[str, Any]:
    ...
    with _LOCK:
        ...
        if inferred_role == "student":
            if "admin" in normalized_email:
                inferred_role = "admin"
            elif "teacher" in normalized_email:
                inferred_role = "teacher"
...
def register_user(name: str, email: str, role: str, password: str | None = None) -> dict[str, Any]:
    normalized_email = (email or "").strip().lower()
